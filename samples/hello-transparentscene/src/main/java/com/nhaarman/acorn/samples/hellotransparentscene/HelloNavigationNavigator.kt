...
import com.nhaarman.acorn.navigation.experimental.TransparentNavigator
...
class HelloNavigationNavigator : TransparentNavigator(null),
    FirstScene.Events,
    SecondScene.Events {

    override fun createInitialScene(): Scene<out Container> {
        return FirstScene(this)
    }
    ...
    override fun instantiateScene(sceneClass: KClass<out Scene<*>>, state: SceneState?): Scene<out Container> {
        return when (sceneClass) {
            FirstScene::class -> FirstScene(
                this
            )
            SecondScene::class -> SecondScene(
                this
            )
